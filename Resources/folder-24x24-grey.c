#include "lvgl/lvgl.h"

#ifndef LV_ATTRIBUTE_MEM_ALIGN
#define LV_ATTRIBUTE_MEM_ALIGN
#endif

#ifndef LV_ATTRIBUTE_IMG_FOLDER-24X24-GREY
#define LV_ATTRIBUTE_IMG_FOLDER-24X24-GREY
#endif

const LV_ATTRIBUTE_MEM_ALIGN LV_ATTRIBUTE_IMG_FOLDER-24X24-GREY uint8_t folder-24x24-grey_map[] = {
  , 
};

const lv_img_dsc_t folder-24x24-grey = {
  .header.always_zero = 0,
  .header.w = 24,
  .header.h = 24,
  .data_size = 1654,
  .header.cf = LV_IMG_CF_RAW,
  .data = folder-24x24-grey_map,
};
